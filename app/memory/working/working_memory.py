...
Keys:
    working:{user_id}:{session_id}:turns   → JSON list of turns (TTL: 2h)
    working:{user_id}:{session_id}:summary → compressed summary string (TTL: 2h)
    working:{user_id}:{session_id}:meta    → {user_id, turn_count} (TTL: 2h)
"""
import json
import logging
...
class WorkingMemory:
    ...
    TURNS_KEY = "working:{user_id}:{session_id}:turns"
    SUMMARY_KEY = "working:{user_id}:{session_id}:summary"
    META_KEY = "working:{user_id}:{session_id}:meta"
    ...
    def _turns_key(self, user_id: str, session_id: UUID) -> str:
        return self.TURNS_KEY.format(user_id=user_id, session_id=str(session_id))

    def _summary_key(self, user_id: str, session_id: UUID) -> str:
        return self.SUMMARY_KEY.format(user_id=user_id, session_id=str(session_id))

    def _meta_key(self, user_id: str, session_id: UUID) -> str:
        return self.META_KEY.format(user_id=user_id, session_id=str(session_id))
    ...
    async def add_turn(
        self, user_id: str, session_id: UUID, role: str, content: str
    ) -> ConversationTurn:
        """Append a turn to working memory. Triggers compression if needed."""
        turns_key = self._turns_key(user_id, session_id)
        meta_key = self._meta_key(user_id, session_id)
    ...
    async def get_turns(self, user_id: str, session_id: UUID) -> List[ConversationTurn]:
        """Retrieve current turns for a session."""
        raw = await self.redis.get(self._turns_key(user_id, session_id))
        if not raw:
            return []
        return [ConversationTurn(**t) for t in json.loads(raw)]

    async def get_summary(self, user_id: str, session_id: UUID) -> Optional[str]:
        """Return compressed summary if it exists."""
        raw = await self.redis.get(self._summary_key(user_id, session_id))
        return raw if raw else None

    async def _compress_to_summary(
        self, user_id: str, session_id: UUID, turns: List[dict]
    ) -> None:
...
SUMMARY:"""
        ...
        try:
            ...
            logger.info("working_memory_compressed", extra={"session_id": str(session_id), "user_id": user_id})
    ...
    async def get_context_for_prompt(
        ...
        turns = await self.get_turns(user_id, session_id)
        summary = await self.get_summary(user_id, session_id)
    ...
    async def clear_session(self, user_id: str, session_id: UUID) -> None:
        """Delete all working memory for a session."""
        pipe = self.redis.pipeline()
        pipe.delete(self._turns_key(user_id, session_id))
        pipe.delete(self._summary_key(user_id, session_id))
        pipe.delete(self._meta_key(user_id, session_id))
        await pipe.execute()
        logger.info("session_cleared", extra={"session_id": str(session_id), "user_id": user_id})
